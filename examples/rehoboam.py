...
def xmaslight():
    ...
    for i in range(0, numBands):
        ...
        newBand = list(
            filter(lambda c: c[2] >= bottom + i * bandHeight and c[2] < bottom + (i + 1) * bandHeight and c[2] <= top,
                   usedCoords))
    ...
    while True:
        ...
        if currentTime > (lastFade + fadeInterval):  # fading first, so that the pixels get to light fully before fading
            for i in range(0, len(redValues)):
                ...
                if redValues[i] > 0:
                    redValues[i] -= int((color[1] / fadeTime) * (
                                currentTime - lastFade))  # decrement red value based on amount of time since last decrement for smoothness
                    if redValues[i] < 0:
        ...
        if currentBands < bandLimit:  # add a random new band until we hit the limit
            ...
            if not bandsInUse[bandIndex]["inUse"]:
                bandsInUse[bandIndex] = {
                    ...
                    # in radians per second; the rotational speed at which the "band marker" travels
                }
                currentBands += 1
        for b in bandsInUse.keys():
            ...
            if band["inUse"]:
                if currentTime - band["startTime"] <= band["timeToRun"]:  # if band hasn't yet run for its full duration
                    ...
                    coordsToLight = list(
                        filter(lambda c: findAngle(c) <= band["currentAngle"] and findAngle(c) >= previousAngle,
                               bands[b]))
